...
linear_model_oil_all <- 
  lm(Total ~ price_in_usd, lm_data_oil_all)

summary(linear_model_oil_all)

ggplot(lm_data_oil_all, aes(x = price_in_usd, y = Total)) +
  geom_point() +
  theme_minimal()
...
total_data_set_all_av <- 
  inner_join(unemployment_trimmed, lm_data_oil_all, by = "Year")

ggplot(total_data_set_all_av, aes(unemployment_rate, Total)) + 
  geom_point() + 
  theme_minimal()

unemployment_rate_lm_all <- 
  lm(Total ~ unemployment_rate, total_data_set_all_av )

summary(unemployment_rate_lm_all)
...
lm_data_inflation <- 
  inner_join(yearly_vehicle_sales, inflation_rates_cleaned, by = "Year")

ggplot(lm_data_inflation, aes(x = inflation_rate, y = Total)) +
  geom_point() +
  theme_minimal()

linear_model_inflation <- 
  lm(Total ~ inflation_rate, lm_data_inflation)

summary(linear_model_inflation)
...
data_set_unemployment_car_sales <- 
  inner_join(unemployment_trimmed, yearly_vehicle_sales, by = "Year")

data_set_unemployment_car_sales_oil <- 
  inner_join(data_set_unemployment_car_sales, oil_trimmed, by = "Year")

lm_oil_unemployment_sales <- 
  lm(Total ~ unemployment_rate + price_in_usd, 
     data = data_set_unemployment_car_sales_oil)
...
#lets throw inflation in too. just to see 

data_set_unemployment_car_sales_oil_inflation <- 
  inner_join(data_set_unemployment_car_sales_oil, inflation_rates_cleaned, by = "Year")

lm_model_total_inflation_too <- 
  lm(Total ~ unemployment_rate + price_in_usd + inflation_rate, 
     data = data_set_unemployment_car_sales_oil_inflation)

summary(lm_model_total_inflation_too)
