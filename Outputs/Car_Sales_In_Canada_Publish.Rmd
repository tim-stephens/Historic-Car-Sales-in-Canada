---
title: "In the Early 1980s, Unemployment and Inflation Rates were Strongly Correlated with Low Automobile Sales, but Skyrocketing Oil Prices were Not" 
author: "Tim Stephens"
...
###PREAMBLE###
# This analysis focuses on trying to determine what caused the drastic reduction in sales for the Canadian automobile market in the early 1980s. Four datasets will be imported, cleaned, and analyzed to determine possible relationships. These four datasets are: new vehicle sales, world oil prices, unemployment rates, and inflation rates. 
...
## 1.3 Structure of the Paper and Methods of Analysis

|       This paper will try to confirm the research above using datasets provided by various sources, which are discussed in the next section. A surface level investigation into Canadian automobile sales, world oil prices, Canadian unemployment rates, and Canadian inflation levels will then be done. After, four regression discontinuity design (RDD) models will be constructed to observe the dip in sales during the early 1980s. Three separate linear regression models will then be constructed with vehicle sales as the dependent variable and oil prices, unemployment rate, and inflation rate used as the independent variables. A multiple linear regression model will also be created. The paper will then discuss results, key findings, bias, weaknesses, and some next steps. Although this study looks to draw a link between vehicle sales, oil prices, unemployment rates, and inflation, it will not be able to speak to causality. The reasoning behind why causality cannot be determined will be talked about in the discussion section, after key findings.  
...
|       The last dataset for Canadian inflation rates was created by the analyst. Data was pulled from inflation.eu (@infeu1). Inflation.eu "contains current and historic data about inflation and the consumer price index", citing Statistics Canada as its source for Canadian data (@infeu2). Data was copied from the historic data web page into a csv file in order to run the analysis.
...
```{r Car Sales, fig.cap="Historical Car Sales in Canada", echo = FALSE, message = FALSE, fig.align='center', out.width="63%", out.height="63%"}
...
```{r Truck Sales, fig.cap="Historical Truck Sales in Canada", echo = FALSE, message = FALSE, fig.align='center', out.width="63%", out.height="63%"}
options(scipen=999) #removes scientific notation
...
```{r Vehicle Sales, fig.cap="Historical Total Automobile Sales in Canada", echo = FALSE, message = FALSE, fig.align='center', out.width="63%", out.height="63%"}
options(scipen=999) #removes scientific notation
...
```
|       The combined plot in Figure 3 looks as one might expect: similar to the car sales plot, with less dramatic peaks and troughs. The truck sales have made the market less volatile. For the rest of the paper, this will be the dataset used when discussing vehicle sales in Canada. Generally, sales have increased but saw a dramatic decrease from 1979-1982. To see the same plot with a variable x-axis, use: https://tim-stephens1.shinyapps.io/Auto_Sales_Adjustable_X_Axis/. The adjustable x-axis has the ability to show the dramatic decrease in sales by trimming out more recent years. Now that vehicle sales are known, independent variables of interest were selected to see if they were related to sales. The first is historic oil prices. Although Canadian oil prices could not be found in the correct time frame, world oil prices follow similar trends, and will be sufficient for this study. Figure 4 shows historic oil prices of OPEC crude oil.

```{r Oil Plot, fig.cap="Historical Oil Prices", echo = FALSE, message = FALSE, fig.align='center', out.width="63%", out.height="63%"}
...
```
|       Unemployment data was only available from 1976, so the x-axis scale is different than Figures 1-4. Figure 5 shows that unemployment rates have fluctuated over time, but it is normal for countries to experience periods with high unemployment. There were two dramatic peaks for Canadians: 1983 and 1993. 1983 saw the highest unemployment rate at 12%, but unemployment was also very high in 1982. This is an extremely interesting point, as it falls within the scope of the study. To see the same plot with an adjustable x-axis, use: https://tim-stephens1.shinyapps.io/Unemployment_Rate_Adjustable_Range/. Having a variable x-axis allows one to trim off more recent years. The last variable of interest was inflation rate. Historic levels are shown in Figure 6. 


```{r Inflation Plot, fig.cap="Inflation Rates in Canada", echo = FALSE, message = FALSE, fig.align='center', out.width="63%", out.height="63%"}
...
|       As stated in the introduction, the Bank of Canada tries to keep inflation between 1 and 3%, which can be seen in Figure 6 from 1991 onward. Before then, inflation seemed to run wild, peaking in 1974 with inflation at 12.65% (@infeu1). Inflation was also very high during the period of interest, with inflation at 12.12% in 1981 (@infeu1). To see the same plot with an adjustable x-axis, use: https://tim-stephens1.shinyapps.io/Inflation_rates_adjustable_range/. The changeable x-axis shows the dramatic increase in inflation during the period of interest by trimming off more recent years. To easily compare all variables, Figure 7 was created. It shows the total number of vehicle sales, world oil prices, unemployment rates, and inflation rates. 


```{r All Plots of Interest Together, fig.cap="Total Automobile Sales, Oil Prices, Unemployment Rates, and Inflation Rates", echo = FALSE, message = FALSE, fig.align='center', out.width="72%", out.height="72%"}
...
grid.arrange(vehicle_plot, oil_plot, unemployment_plot, inflation_plot, 
             ncol = 2, 
             top = "              In the Early 1980s, Vehicle Sales were Low, the Price of Oil was High, 
...
```{r RDD1980, fig.cap="RDD for Oil Prices", echo = FALSE, message = FALSE, fig.align='center', out.width="64%", out.height="64%"}

#Total RDD split on year 1980
#Code for RDD model is based on code from INF2178H at the University of Toronto, instructed by Rohan Alexander. A link to the code is below.
#https://www.tellingstorieswithdata.com/causality-from-observational-data.html#regression-discontinuity-design 

yearly_vehicle_sales %>% 
  ggplot(aes(x = Year,
             y = Total)) +
  geom_point(alpha = 0.2) + #increases transparency of points
  #create the cut-off at 1980. All points before 1980 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year < 1980), 
              method='lm',
              color = "black") +
  #all points after 1980 are grouped together: 
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year >= 1980), 
              method='lm',
              color = "black") +
  theme_minimal() +
...
```{r RDD1983, fig.cap="RDD for Unemployment Rates", echo = FALSE, message = FALSE, fig.align='center', out.width="64%", out.height="64%"}

#Total RDD split on year 1983
#Code for RDD model is based on code from INF2178H at the University of Toronto, instructed by Rohan Alexander. A link to the code is below.
#https://www.tellingstorieswithdata.com/causality-from-observational-data.html#regression-discontinuity-design 
...
yearly_vehicle_sales %>% 
  ggplot(aes(x = Year,
             y = Total)) +
  geom_point(alpha = 0.2) + #increases transparency of points
  #create the cut-off at 1983. All points before 1983 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year < 1983), 
              method='lm',
              color = "black") +
  #All points after 1983 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year >= 1983), 
              method='lm',
              color = "black") +
  theme_minimal() +
...
```{r RDD1981, fig.cap="RDD for Inflation Rates", echo = FALSE, message = FALSE, fig.align='center', out.width="65%", out.height="65%"}

#Total RDD split on year 1981
#Code for RDD model is based on code from INF2178H at the University of Toronto, instructed by Rohan Alexander. A link to the code is below.
#https://www.tellingstorieswithdata.com/causality-from-observational-data.html#regression-discontinuity-design 
...
yearly_vehicle_sales %>% 
  ggplot(aes(x = Year,
             y = Total)) +
  geom_point(alpha = 0.2) + #increases transparency of points
  #create the cut-off at 1981. All points before 1981 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year < 1981), 
              method='lm',
              color = "black") +
  #All points after 1981 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year >= 1981), 
              method='lm',
              color = "black") +
  theme_minimal() +
...
yearly_vehicle_sales %>% 
  ggplot(aes(x = Year,
             y = Total)) +
  geom_point(alpha = 0.2) + #increases transparency of points
  #Create the cut-off point. All points before 1982 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year < 1982), 
              method='lm',
              color = "black") +
  #All points after 1982 are grouped together:
  geom_smooth(data = yearly_vehicle_sales %>% filter(Year >= 1982), 
              method='lm',
              color = "black") +
  theme_minimal() +
...
#join two datasets together, based on year. We need them in one dataset so we can run analysis on them.
...
ggplot(lm_data_oil_yearly_vehicle_sales, aes(x = price_in_usd, y = Total)) +
  ...
  #lm method gives us same line as calculated in the regression model
  theme_minimal() + 
  scale_x_continuous(labels=scales::dollar_format())+ #dollar format on x-axis
...
#create a nice output table of findings

huxreg(" " = linear_model_oil_and_sales,
       coefs = c("Intercept" = "(Intercept)", 
                 "Oil Price (USD)" = "price_in_usd"), #renaming 
       statistics = c("Number of Observations" = "nobs", #choose statistics we want to show
                      "R Squared" = "r.squared",
                      "P Value" = "p.value"),
       error_pos = 'right') %>% #change error position to the right (instead of below)
  set_italic(final(1),1) %>% #italicize p value footnote
  set_caption("Linear Regression of Automobile Sales and Oil Prices") %>% #add a title
  add_footnote("Values in parentheses represent standard error") #add a footnote
...
ggplot(oil_sales_unemp_dataset, aes(x = unemployment_rate, y = Total)) +
  geom_point() +
  geom_smooth(method = 'lm', col = 'blue') +
  #lm line gives us same line as calculated using linear regerssion above
  theme_minimal() + 
  scale_x_continuous(labels=scales::percent_format(scale = 1)) + # gives % on x-axis 
...
huxreg(" " = unemployment_rate_lm_sales,
       coefs = c("Intercept" = "(Intercept)", 
                 "Unemployment Rate" = "unemployment_rate"), #rename
       statistics = c("Number of Observations" = "nobs", #choose statistics we want
                      "R Squared" = "r.squared",
                      "P Value" = "p.value"),
       error_pos = 'right') %>%  #change error position
  set_italic(final(1),1) %>%  #italicize p value note
  set_caption("Linear Regression of Automobile Sales and Unemployment Rates") %>% #add a title
  add_footnote("Values in parentheses represent standard error") #add a footnote
```
\newpage
...
#join two datasets together on year
...
ggplot(oil_sales_unemp_inf_dataset, aes(x = inflation_rate, y = Total)) +
  geom_point() +
  geom_smooth(method = 'lm', col = 'blue') +  
  #lm line allows us to see the same regression line calculate above
  theme_minimal() +
  scale_x_continuous(labels=scales::percent_format(scale = 1)) + #gives us % on x-axis 
  labs(x = "Inflation",
       y = "Total Number of Vehicles Sold",
...
#make a nice table to report regression findings

huxreg(" " = linear_model_inflation_sales,
       coefs = c("Intercept" = "(Intercept)", 
                 "Inflation Rate" = "inflation_rate"), #rename
       statistics = c("Number of Observations" = "nobs", #choose statistics to show
                      "R Squared" = "r.squared",
                      "P Value" = "p.value"),
       error_pos = 'right') %>% #change error position
  set_italic(final(1),1) %>% #italicize p value note
  set_caption("Linear Regression of Automobile Sales and Inflation Rate") %>% #title
  add_footnote("Values in parentheses represent standard error") #add a footnote
...
#linear regression for all three models 
#don't need to join, b/c all 4 datasets have been combined above. 
...
huxreg(" " = multiple_lm_model_all_3,
       coefs = c("Intercept" = "(Intercept)",
                 ...
                 "Inflation Rate" = "inflation_rate"),   #rename              
       statistics = c("Number of Observations" = "nobs", #what statistics do we want to see?
                      "R Squared" = "r.squared",
                      "P Value" = "p.value"),
       error_pos = 'right') %>% #change error position
  set_italic(final(1),1) %>%  #italicize the p value note
  set_caption("Multiple Linear Regression of Automobile Sales, Oil Price, Unemployment Rate, and Inflation Rate") %>% #title
  add_footnote("Values in parentheses represent standard error")#footnote
```
\newpage
...
|       An interesting hypothesis based on literature review is that oil prices indirectly affected other factors that are associated with automobile sales. Even though oil prices were not significantly correlated to automobile sales, it is possible that the skyrocketing price of oil launched a global recession, which led to an increase in unemployment and inflation rates in Canada. So, even though oil prices are not directly associated with a decrease in sales, it is possible that increased prices still had a role in the story. 
...
|       In this study, though, time was used as the independent variable. Time itself cannot be used for causation. A year is not a reason for something to happen. Multiple things occur in any given year, and it is those variables that cause changes in dependent variables. In this study, RDD was used to inspect different years in the early 1980s that experienced drops in automobile sales. The other variables are what led to the drop in car sales. However, the other variables did not have a distinct cut-off point to examine, and thus, an RDD model could not be designed to inspect causality between oil prices, unemployment rates, inflation, and vehicle sales.

|       After conducting research into the socioeconomic factors of the 1970s and early 1980s, it is likely that the skyrocketing oil prices are what led to the Canadian recession, ultimately increasing unemployment rates and inflation. With no jobs and their money worth less than it was in the past, Canadians were not in the market to purchase new vehicles. The multiple linear regression model supports this hypothesis, as oil prices lose significance after controlling for unemployment and inflation (showing that there is some interaction between variables), and unemployment and inflation are significantly associated with automobile sales. Statistically, though, there is no evidence to show that the increased price of oil is what caused increased unemployment and inflation.  
...
# 6.0 Conclusion 
|       This paper inspected a specific time frame of Canadian automobile sales. Although the sector has seen growth since its inception, there have been a few periods of drastic decreases in sales, such as the early 1980s. To determine why sales dropped, literature review was conducted, and three variables of interest were determined: oil prices, unemployment rates, and inflation rates. Each variable was inspected at a surface level, and it was found that oil prices doubled in the late 1970s and both unemployment and inflation were high during the early 1980s. RDD models were designed to see the drop in sales, showing that there was a significant change during the early 1980s. After, linear models were produced, with each factor being significantly correlated to sales individually. But, after running a multiple linear regression, it was determined that oil was interacting with unemployment and inflation behind the scenes. It was found that after controlling for unemployment and inflation, oil prices were no longer correlated to sales. Although this paper cannot speak to causality from a statistical point of view, it is likely that the doubling of oil prices in the late 1970s triggered a recession in Canada, ultimately leading to high unemployment and inflation rates which are correlated to low automobile sales. In the future, models will be designed to determine if oil is, in fact, what caused an increase in inflation and unemployment, and if unemployment and inflation are what caused the decrease in automobile sales. For now, it can only be stated that these factors are correlated to one another. Additionally, other time periods will be studied to further increase the strength of the model, with the goal of predicting future automobile sales using multiple variables. Unemployment rates and inflation are a good start, but other socioeconomic factors likely affect sales as well. 
